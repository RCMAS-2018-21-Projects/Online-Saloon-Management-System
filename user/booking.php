...
<label>CHOOSE</label></td>
   <?php 
     $sql="select sub_cat from sub_category where category='$a' ";
        $tbl=getDatas($sql);
    ?>
...
<label><input type="radio" value="14:15 pm" name="time">
    ...
    <label><input type="radio" value="15:45 pm" name="time">
...
if(isset($_POST['bid'])){
 ...
 where servicename	='$doc'";
	$tbl=getDatas($sql);
	$pr=$tbl[0][0];
    $sql="select time1 from tbl_booking where time1='$time' and date1='$newdate'";
    $tbl=getDatas($sql);
     if($v < $t && $d == $newdate){
        msgbox('Select another slot .');
              ...
              msgbox('Sorry... No time slot available! try another one.');

    }
    else
    {
        $sql="insert into payment(purpose,date,amount,email) values('$doc','$newdate','$pr','$user')";
setDatas($sql);

        
        
    $sql="insert into `tbl_booking`(`customer`,`email`, `category`, `sub_cat`, `date1`, `time1`, `price`,`status`) values('$pat','$user','$a','$doc','$newdate','$time','$pr','Active')" ;
    setDatas($sql);
